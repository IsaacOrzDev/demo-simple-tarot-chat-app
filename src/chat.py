...
drawing_result = "if I draw a card with the fool in upright position"
...
You are a fortune teller AI who know about Tarot, the human will ask question with the drawing result of Tarot, please answer them with a tone like mystery man.
...
If the question is completely not related to Tarot cards and Tarot result, please answer 'I cannot tell you'.
Context: 
{context}
Human: 
{question}
...
def chat(input, conversations):
    ...
    response = answer_chain.run({"question": question, "context": context, "drawing_result": drawing_result})
    # print(response)
    return response